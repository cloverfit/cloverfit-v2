  ...
  if (subjectiveScore != null) {
    ...
    if (objLevel === 'high' && subLevel === 'high') {
      alignmentFeedback = {
        type: 'aligned-high',
        title: '心身ともに絶好調',
        summary: '心拍データも体の実感も、どちらも良好な状態を示しています。',
        detail: '心拍の回復力と、あなた自身が感じるコンディションがしっかり一致しています。体の内側も外側も、いい状態ですね。',
      ...
      alignmentFeedback = {
        type: 'aligned-mid',
        title: '安定したコンディション',
        summary: '心拍データと体の実感が、どちらも安定しています。',
        detail: '体の感覚と心拍データが合っているのは、自分の状態をよく把握できている証拠です。このバランス感覚は大きな強みです。',
      ...
      alignmentFeedback = {
        type: 'aligned-low',
        title: 'しっかり休養を取りましょう',
        summary: '心拍データも体の実感も、疲れが溜まっていることを示しています。',
        detail: '心身ともに疲れが出ている時期かもしれません。無理をせず、まずは回復を最優先にしましょう。',
        advice: '睡眠時間の確保（7〜9時間）とリラックスできる時間を優先してください。しっかり回復してからのトレーニングの方が、効果も高くなります。',
      }
    } else if ((objLevel === 'high' || objLevel === 'mid') && subLevel === 'low') {
      alignmentFeedback = {
        type: 'body-ahead',
        title: '体は元気、でも気持ちがついてきていないかも',
        summary: '心拍データは良好ですが、体の実感としては疲れを感じています。',
        detail: '心拍の回復力はしっかり維持されているのに、自分自身では疲れを感じている状態です。メンタル面の疲労やストレス、睡眠の質が影響しているかもしれません。',
        advice: '体の回復力は維持できているので、心のケアに目を向けてみましょう。十分な睡眠、リラックスできる趣味の時間、人との会話など、気持ちをリフレッシュすることで実感も追いついてきます。',
      }
    } else if (objLevel === 'low' && (subLevel === 'high' || subLevel === 'mid')) {
      alignmentFeedback = {
        type: 'mind-ahead',
        title: '気分は良いけど、体は少しお疲れかも',
        summary: '体の実感では調子が良いですが、心拍データは疲労のサインを示しています。',
        detail: '自分では調子が良いと感じていても、心拍の回復がまだ追いついていない状態です。気づかないうちに体に負荷がかかっている可能性があります。',
        advice: '調子が良いときこそ要注意です。オーバートレーニングや睡眠不足が隠れている場合があります。軽めの運動に切り替えて、体の回復を優先する日を作ってみてください。',
      }
    } else {
      ...
      if (Math.abs(gap) < 20) {
        alignmentFeedback = {
          type: 'aligned-mid',
          title: 'バランスの取れた状態',
          summary: '心拍データと体の実感が概ね一致しています。',
          detail: '体の感覚と心拍データのバランスが取れています。自分の体の状態をしっかり感じ取れていますね。',
          advice: '今のペースを維持しつつ、少しずつ運動の強度や頻度を上げていくことで、着実にレベルアップできます。',
        }
      } else if (gap > 0) {
        alignmentFeedback = {
          type: 'mind-ahead',
          title: '体の実感は良好、でもデータは注意信号',
          summary: '自分の感覚に対して、心拍データがやや追いついていません。',
          detail: '体感では好調ですが、心拍の回復にはもう少し余地がありそうです。見えない疲れが隠れているかもしれません。',
          advice: '気分が良いのは素晴らしいことです。ただ、心拍データが示すサインにも耳を傾けて、休養日を適度に入れましょう。',
        }
      } else {
        alignmentFeedback = {
          type: 'body-ahead',
          title: 'データは好調、実感はもう少し',
          summary: '心拍データは良い状態ですが、体の実感がまだ追いついていません。',
          detail: '心拍の回復力は順調ですが、自分自身ではまだ疲れを感じている状態です。ストレスや精神的な疲労が影響しているかもしれません。',
